...
import org.ape.util.EPSGraphics2D;
...
public class EpsFileType extends FileType {
	...
	@Override
	public Icon getIcon() {
		final Icon icon = GraphicsTools.getIcon("ape/filechooser/eps.gif");
		return icon;
	}
	...
	public void save(Document document, File file) throws FileTypeException {
		try {
			...
			for (Element element : document.petriNet.getCurrentSubnet().getElements()) {
				element.getGraphicalElement().draw(epsGraphics2d, drawingOptions);
			}
